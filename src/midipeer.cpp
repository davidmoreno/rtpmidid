...
void midipeer_t::process_midi_packet(const rtpmidid::midi_packet_t &packet) {
  ...
  const auto send_time_us =
      std::chrono::duration_cast<std::chrono::microseconds>(send_end - send_start)
          .count();
  DEBUG(
      "[TIMING] peer {}: MIDI packet processed - total: {} us ({:.2f} ms), "
      "send_midi: {} us, size: {} bytes",
      peer_id, processing_time_us, processing_time_us / 1000.0, send_time_us,
      packet.data.size());
#endif
}
