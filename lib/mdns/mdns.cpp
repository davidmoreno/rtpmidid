...
#ifndef MDNS_CUSTOM
# error "Compilation of this file needs -DMDNS_CUSTOM"
#endif
...
static ::mdns::ip4_t route_get_ip_for_route(struct in_addr);
static ::mdns::ip4_t guess_default_ip();

mdns::mdns::mdns(const std::string &default_ip) {
  ...
  rtpmidid::poller.add_fd_in(socketfd, [this](int) { this->mdns_ready(); });
...
bool read_question(mdns::mdns *server, ::mdns::parse_buffer_t &buffer) {
  uint8_t label[128];
  ::mdns::parse_buffer_t parse_label = {label, label + sizeof(label), label};
  ...
  return server->answer_if_known(mdns::mdns::query_type_e(type_), (char *)label);
}

bool read_answer(mdns::mdns *server, ::mdns::parse_buffer_t &buffer) {
  uint8_t label[128];
  ::mdns::parse_buffer_t buffer_label = {label, label + 128, label};
  ...
  if (type_ == mdns::mdns::PTR) { // PTR
    uint8_t answer[128];
    ::mdns::parse_buffer_t buffer_answer = {answer, answer + 128, answer};
    read_label(buffer, buffer_answer);
    // DEBUG("PTR Answer: {} {} {} -> <{}>", label, type_, class_, answer);
    mdns::mdns::service_ptr service((char *)label, ttl, (char *)answer);
    server->detected_service(&service);
  } else if (type_ == mdns::mdns::SRV) { // PTR
    ...
    uint8_t target[128];
    ::mdns::parse_buffer_t buffer_target = {target, target + 128, target};
    read_label(buffer, buffer_target);

    mdns::mdns::service_srv service((char *)label, ttl, (char *)target, port);
    server->detected_service(&service);
...
void mdns::mdns::on_discovery(const std::string &service, mdns::mdns::query_type_e qt,
                        std::function<void(const mdns::mdns::service *)> f) {
...
void mdns::mdns::remove_discovery(const std::string &service,
                            mdns::mdns::query_type_e type) {
  discovery_map.erase(std::make_pair(type, service));
}

void mdns::mdns::query(const std::string &service, mdns::mdns::query_type_e qt,
                 std::function<void(const mdns::mdns::service *)> f) {
  if (service.length() > 100) {
    throw rtpmidid::exception(
        "Service name too long. I only know how to search for smaller names.");
...
void mdns::mdns::announce(std::unique_ptr<service> service, bool broadcast) {
...
void mdns::mdns::reannounce_later(service *srv) {
  // DEBUG("Will reannounce in {}s", srv->ttl);
  auto timer_id = rtpmidid::poller.add_timer_event(srv->ttl * 1s, [this, srv] {
...
void mdns::mdns::unannounce(service *srv) {
  srv->ttl = 0;
  send_response(*srv);
...
void mdns::mdns::send_response(const service &service) {
  uint8_t packet[1500];
  memset(packet, 0, 16); // Here there are some zeros, the rest will be
                         // overwritten or not used
  ::mdns::parse_buffer_t buffer = {packet, packet + 1500, packet};
  ...
  switch (service.type) {
  case mdns::mdns::A: {
    auto a = static_cast<const mdns::mdns::service_a *>(&service);
  ...
  case mdns::mdns::PTR: {
    auto ptr = static_cast<const mdns::mdns::service_ptr *>(&service);
  ...
  case mdns::mdns::TXT: {
    auto srv = static_cast<const mdns::mdns::service_txt *>(&service);
    write_label(buffer, srv->txt);
  } break;
  default:
    throw rtpmidid::exception("I dont know how to announce this mDNS answer type: {}",
                    service.type);
  }
...
int mdns::mdns::broadcast(const ::mdns::parse_buffer_t *buffer) {
  return ::sendto(socketfd, buffer->start, buffer->capacity(), MSG_CONFIRM,
                  (const struct sockaddr *)&multicast_addr,
                  sizeof(multicast_addr));
}

bool mdns::mdns::answer_if_known(mdns::mdns::query_type_e type_, const std::string &label) {
...
void mdns::mdns::query(const std::string &name, mdns::mdns::query_type_e type) {
  ...
  ::mdns::parse_buffer_t buffer = {packet, packet + 120, packet + 12};
...
void parse_packet(mdns::mdns *mdns, ::mdns::parse_buffer_t &parse_buffer) {
...
void mdns::mdns::mdns_ready() {
  ...
  *((int32_t *)&ip4) = ip;
  ...
  ::mdns::parse_buffer_t parse_buffer{
      buffer,
      buffer + read_length,
      buffer,
  };
...
void mdns::mdns::detected_service(const mdns::mdns::service *service) {
  ...
  // item from the loop).
  std::vector<std::function<void(const mdns::mdns::service *)>> tocall;
...
void mdns::mdns::update_cache(const mdns::mdns::service *service) {
  ...
  mdns::mdns::service *service_at_cache = nullptr;
...
std::string mdns::mdns::local() {
  char hostname[256];
  gethostname(hostname, sizeof(hostname));
  return fmt::format("{}.local", hostname);
}

std::string std::to_string(const mdns::mdns::service_ptr &s) {
  return fmt::format("PTR record. label: {}, pointer: {}", s.label,
                     s.servicename);
}
std::string std::to_string(const mdns::mdns::service_a &s) {
  return fmt::format("A record. label: {}, ip: {}.{}.{}.{}", s.label,
                     uint8_t(s.ip[0]), uint8_t(s.ip[1]), uint8_t(s.ip[2]),
                     uint8_t(s.ip[3]));
}
std::string std::to_string(const mdns::mdns::service_srv &s) {
...
static ::mdns::ip4_t route_get_ip_for_route(struct in_addr in_other) {
  union ip_t {
    uint8_t ip4p[4];
    ::mdns::ip4_t ip4;
  };
