...
`midirouter_t::status()` SHALL aggregate peer statuses as `std::vector<std::variant<...>>` (one variant entry per peer), enriching each with router-assigned members via generic visit, without constructing any ad-hoc JSON. The serialized `router` array SHALL be byte-compatible with today's. Aggregation SHALL be asynchronous and requester-driven: the router answers the requester with a status head carrying router-side statistics and the router-assigned members of each expected peer and scatters the status request with the requester's `reply_to`; each peer produces its typed status variant from its own thread-local state and replies directly to the requester; the requester merges the responses under its own deadline (partial results permitted, unresponsive peers marked). The router SHALL keep no gather state.
