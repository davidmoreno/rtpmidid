...
The actor execution model: one thread/poller/mailbox per actor, mailbox-only cross-thread communication, doorbell wake protocol, configurable drain policies, selective waits, lifecycle management, scheduling priority configuration, and threadless test pumping.
