...
Detect and report message handlers that block an actor's loop: the actor runtime measures each message handler invocation and logs an ERROR when one exceeds a per-actor threshold, naming the actor and the message.
