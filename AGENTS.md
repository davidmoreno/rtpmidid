...
- **Router queue** (`midirouter.hpp`): `priority_mpsc_queue<router_command_t, 4096, 256, 64>` carrying a `std::variant` of **per-purpose typed structs** — every operation has its own named alternative, never a generic “run this lambda” / “fire this signal”. The router thread drains HIGH first (MIDI), then NORMAL (topology + signals), then LOW (read snapshots).
  - HIGH: `send_midi_t`.
  - NORMAL: `add_peer_t`, `remove_peer_t`, `remove_all_peers_t`, `connect_t`, `disconnect_t`, `event_directed_t`, `event_broadcast_t`; signal dispatch `signal_peer_added_t`, `signal_connected_t`, `signal_disconnected_t`, `signal_peer_event_t`; `shutdown_t`.
...
- **Peer queue** (`midipeer.hpp`): `priority_mpsc_queue<peer_command_t, 1024, 64, 16>` of `process_midi_t` (HIGH), `shutdown_t` (NORMAL), `query_internal_latency_stats_t` (LOW). Producers are the router thread (MIDI) and any other thread issuing latency queries.
